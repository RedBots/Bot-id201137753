...
function getLatestCommit() {
    window.jQuery.ajax({
        ...
        $.get('https://raw.githubusercontent.com/RedBots/Bot-id201137753/master/bot.user.js?' + Math.floor((Math.random() * 1000000) + 1), function(data) {
            ...
            var myVersion = parseFloat(redBotsVersion + 0.0000); 

            if(latestVersion > myVersion)
            {
                update("redBots", "bot.user.js", "https://github.com/RedBots/Bot-id201137753/blob/" + sha + "/bot.user.js/");
            }
...
(function(f, g) {
    ...
    f.botList.push(["redBots " + redBotsVersion, findDestination]);

    var bList = g('#bList');
    g('<option />', {value: (f.botList.length - 1), text: "redBots"}).appendTo(bList);
